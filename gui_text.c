              ...
              const wchar_t* unicode_text)
{
    ...
    if( glyph_counter > 0 ) {
        ...
        canvas_append_attributes(canvas, SHADER_ATTRIBUTE_COLOR, COLOR_SIZE, GL_UNSIGNED_BYTE, glyph_counter*4, colors);
        canvas_append_attributes(canvas, SHADER_ATTRIBUTE_TEXCOORD, TEXCOORD_SIZE, GL_FLOAT, glyph_counter*4, texcoords);
