...
void bsp_create(struct Mesh* mesh,
                struct Bsp* bsp)
{
    GLuint vertex_id = mesh->vbo->buffer[vertex_array].id;
    GLuint normal_id = mesh->vbo->buffer[normal_array].id;
    GLuint element_id = mesh->elements.id;

    if( vertex_id && normal_id && element_id ) {
