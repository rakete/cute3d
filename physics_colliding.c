...
void colliding_prepare_shape(struct Shape* shape) {
    pivot_combine(shape->pivot.parent, &shape->pivot, &shape->world_pivot);
    pivot_world_transform(&shape->world_pivot, shape->world_transform);
}
...
void colliding_prepare_collision(const struct Shape* a, const struct Shape* b, struct CollisionParameter parameter, struct Collision* collision) {
    pivot_between_transform(&a->world_pivot, &b->world_pivot, collision->shape1_to_shape2_transform);
    pivot_between_transform(&b->world_pivot, &a->world_pivot, collision->shape2_to_shape1_transform);
    collision->shape1 = a;
    collision->shape2 = b;
...
int32_t colliding_contact_convex_convex(struct Collision* collision) {
    ...
    if( result->edge_test.distance > parameter->edge_tolerance * face_separation + parameter->absolute_tolerance ) {
        contacts_halfedgemesh_edge_edge(&result->edge_test, &convex1->base_shape.world_pivot, mesh1, &convex2->base_shape.world_pivot, mesh2, &collision->contacts);
        ...
        if( result->face_test2.distance > parameter->face_tolerance * result->face_test1.distance + parameter->absolute_tolerance ) {
            ...
            contacts_halfedgemesh_face_face(&result->face_test1, &convex1->base_shape.world_pivot, mesh1, &convex2->base_shape.world_pivot, mesh2, &collision->contacts);
        } else {
            ...
            contacts_halfedgemesh_face_face(&result->face_test2, &convex2->base_shape.world_pivot, mesh2, &convex1->base_shape.world_pivot, mesh1, &collision->contacts);
        }
    }
